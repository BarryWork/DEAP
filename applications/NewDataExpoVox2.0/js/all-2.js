...
function loadAnalysisNames() {
  ...
  jQuery.get(
    ...
    function(tsv) {
	  ...
	  if (name.includes("desikan_") && !analysis_names.includes(name.split("desikan_")[0]+"desikan_roi_all")){
	    analysis_names.push(name.split("desikan_")[0]+"desikan_roi_all")
	  }
        })
...
function clearDisplayCheck(time) {
  ...
  //Vertexwise only data
  if(model == "GAMM4-FZ-CR-longitudinal-vertex" && display_file_list["scatter"]){
    fname_temp = display_file_list["scatter"].split("_")
    fname_temp[fname_temp.length - 1] = "tunnel.json";
    fname_temp = fname_temp.join("_");
  }
