...
var user_name = user_name.replace(/\./g, '_');
var composer;
var minC = 0.01;
var maxC = 0.08;
...
jQuery(document).ready(function() {
    ...
    var x = readCookie('MMILSurfaceViewerOptionAdjGeom');
    if (x != undefined) {
        if (x == "true") {
            jQuery('#geometry-enabled').attr('checked', 'checked');
        } else {
            jQuery('#geometry-enabled').removeAttr('checked');
        }
    }
    ...
    jQuery('#colormapDisplay').dblclick(function() {
        if (jQuery('#colormapDialog').dialog('isOpen')) {
            ...
            setTimeout(function() {
                drawExpertColormap(colormap, minVal, maxVal);
            }, 100);
        }
    });
    ...
    jQuery('#animate-predictor').click(function() {
        ...
        var start1 = {
            time: jQuery('#age').slider('option', 'value')
        };
        var end1 = {
            time: 99
        };
    ...
    setTimeout(function() {
        jQuery('#message-text').fadeOut('slow');
    }, 1000);
	...
	jQuery(document).keyup(function (e) {
		...
		if (keycode == 65) { //72 'H' 76 'L", 82 'R', 65 'A', 80 'P', 70 'F'
			...
			if (jQuery('#dialog').dialog('isOpen')) {
				...
				jQuery('#dialog').dialog('option', 'height', 300);
			}
...
function resizeEventFunction() {
    // notify the renderer of the size change
    var width = jQuery('#renderArea').width();
    var height = width / 1.5;
    // update the camera
    camera.aspect = width / height;
    camera.updateProjectionMatrix();
    console.log('in resize : width and height are:' + width + " " + height);
    //console.log('resize event...');
    jQuery('#renderArea canvas').attr('width', width);
    jQuery('#renderArea canvas').attr('height', height);
    jQuery('#colormapDisplay').css('top', height - 100);
...
THREEx.WindowResize = function(renderer, camera) {
    var callback = resizeEventFunction;
...
jQuery("#renderArea").parent().hide();

if (model == "GAMM4-FZ-CR-longitudinal-vertex") {
    jQuery("#renderArea").parent().show();
    init();
    animate();
}
...
function init() {
    ...
    loader.load("/applications/SurfaceViewer/data/left_hemisphere_inflated.vtk", function(geom) {
        geom_lh_inflated = geom;
    });
    loader.load("/applications/SurfaceViewer/data/right_hemisphere_inflated.vtk", function(geom) {
        geom_rh_inflated = geom;
    });
    jQuery.getJSON("/applications/SurfaceViewer/data/ico5_lh_surfmask.json", function(a) {
        surfmask_lh = a;
    });
    jQuery.getJSON("/applications/SurfaceViewer/data/ico5_rh_surfmask.json", function(a) {
        surfmask_rh = a;
    });
    ...
    // download geometry
    loader.load("/applications/SurfaceViewer/data/left_hemisphere.vtk", function(geom) {
        geometry_lh = geom;
        //geometry_lh.vertices = geometry_lh.vertices.slice(0, 2562); // vtk seeems to append the triangle indices to the end of the vertices, cut those off here
        ...
        animate();
        resizeEventFunction(); // try to call this function to resize the renderArea
    });
    loader.load("/applications/SurfaceViewer/data/right_hemisphere.vtk", function(geom) {
        geometry_rh = geom;
        //geometry_rh.vertices = geometry_rh.vertices.slice(0, 2562); // vtk seeems to append the triangle indices to the end of the vertices, cut those off here
        ...
        var attrArray = new Array("icoarea", "thick", "volume", "dv");
        var attrArrayTitle = new Array("Surface area expansion factor vs. atlas",
            ...
            "Change in cortical volume expansion factor over time");
        jQuery('#modality,option').attr('fname', attrArray[request]);
        jQuery('#modality,option').attr('title', attrArrayTitle[request]);

        // find out what the last measure displayed was
        var measure = readCookie('MMILSurfaceViewerMea');
        if (measure == null)
            measure = 1;
        setTimeout(function() {
            switchMeasure(measure);
        }, 500); // initial loading of first measure
    });
    ...
    renderer.setSize(jQuery('#renderArea').width(), jQuery('#renderArea').height());
    ...
    jQuery.ajax({
        url: '/applications/SurfaceViewer/data/colormap.json',
        dataType: 'json',
...
function onMouseDown(event) {
    var x = event.pageX - jQuery(event.target).offset().left;
    var y = event.pageY - jQuery(event.target).offset().top;
    var vector = new THREE.Vector3((x / jQuery('#renderArea').width()) * 2 - 1, -(y / jQuery('#renderArea').height()) * 2 + 1, 0.5);
    ...
    if (uselh) {
        // load the label data
        jQuery.ajax({
            url: '../SurfaceViewer/data/lh_vertex_to_label_formatted.json',
            dataType: 'json',
            success: function(data) {
                ...
                setTimeout(function() {
                    jQuery('#message-text').fadeOut('fast');
                }, 3000);
                setTimeout(function() {
                    updateColormap(minVal, maxVal);
                }, 2000);
    ...
    if (userh) {
        jQuery.ajax({
            url: '../SurfaceViewer/data/rh_vertex_to_label_formatted.json',
            dataType: 'json',
            success: function(data) {
                // set as global variable
                rh_Labels = data;
...
function switchMeasure(selectIndex, filename) {
    ...
    if (selectIndex > 0) { // read in a different dataset (zscores/tscores)
	    ...
	    for (roi_name in a){
            ...
            if (temp["values"][0].length < 50) {
                temp["values"] = transformRoiToVertex(a, "lh")
            }

            temp_min = temp["values"][0].reduce(function(a, b) {
                return Math.min(a, b)
            });
            temp_max = temp["values"][0].reduce(function(a, b) {
                return Math.max(a, b)
            });
            temp_abs_max = Math.max(Math.abs(temp_min), Math.abs(temp_max));
            temp["windowLevel"] = [temp_abs_max * -1, temp_abs_max]
            a = temp;
            //jQuery.getJSON("/applications/DataExploration/user_data/vertexdata_main_effect_and_interaction_" + project_name + "_" + user_name + "_" + cookie + "_" + f + "-lh.json", function(a) {
    ...
    jQuery.get("/applications/DataExploration/user_data/vertexdata_stats_" + project_name + "_" + user_name + "_" + cookie + ".txt", function(data) {
        var oldContent = jQuery('#summary').html();
        jQuery('#summary').html(oldContent + "<br/>" + data);
    });
}
...
function updateGeometry(x) {
    var showGeom = jQuery('#geometry-enabled').attr('checked');
    if (typeof showGeom == "undefined") {
        createCookie('MMILSurfaceViewerOptionAdjGeom', false, 7);
        showGeom = false;
    } else {
        createCookie('MMILSurfaceViewerOptionAdjGeom', true, 7);
        showGeom = true;
    }
...
function updateColormap(minV, maxV, highlightMaterial) {
    ...
    if (!geometry_lh) {
        console.log("return since geometry_lh does nto exist.");
        return;
    }
    ...
    if (typeof leftValues.FDR_Threshold != "undefined" &&
        time < leftValues.FDR_Threshold.length) {
    ...
    for (var i = 0; i < geometry_lh.faces.length; i++) {
        ...
        // scale linear in the colormap
        if (time > -1 && surfmask_lh.values[idx1] == 1 && surfmask_lh.values[idx2] == 1 && surfmask_lh.values[idx3] == 1) {
            if (fdrt == true) {
...
function drawExpertColormap(cm, minV, maxV) {
    ...
    for (var i = 0; i < w; i += 2) {
        ...
        var color = 'rgba(' + colors[i][0] + ',' + colors[i][1] + ',' + colors[i][2] + ',' + colors[i][3] + ')';
...
function drawColormap(colormap, minV, maxV) {
    ...
    // get a scale factor for the colormap display based on the size of the render window
    var parentsizeW = jQuery('#renderArea').width() / 5.0;
    var parentsizeH = jQuery('#renderArea').width() / 20.0;
    if (parentsizeW > 200)
        parentsizeW = 200;
    if (parentsizeH > 60)
        parentsizeH = 60;
    jQuery('#colormapDisplay').attr('width', parentsizeW);
    jQuery('#colormapDisplay').attr('height', parentsizeH);
    ...
    for (var i = 0; i < w; i++) {
        ...
        context.moveTo(i, 0);
        context.lineTo(i, h);
        context.strokeStyle = 'rgba(' + colors[i][0] + ',' + colors[i][1] + ',' + colors[i][2] + ',' + colors[i][3] + ')';
        context.stroke();
    }
    var oldfill = context.fillStyle;
    context.fillStyle = 'rgba(255,255,255,0.5)';
    context.font = "12px Arial";
    var down = parentsizeH - 5;
    context.fillText(parseFloat(minV).toFixed(3), 0, down);
    context.fillText(parseFloat(maxV).toFixed(3), w - 30, down);
...
function transformRoiToVertex(data, label) {
    // load lh vtk mapping file
    var rt = []
    jQuery.ajax({
        url: '../SurfaceViewer/data/' + label + '_vertex_to_label_formatted.json',
        dataType: 'json',
        async: false,
        success: function(map) {
            ...
            map_dict = {}
            map_dict_reverse = {}
            for (item in map["names"]) {
                tuple = map["names"][item]
                tuple_code = tuple[0]
                tuple_name = tuple[1]
                map_dict[tuple_name] = {
                    code: tuple[0],
                ...
                map_dict_reverse[tuple[0]] = tuple_name
            }

	    console.log(map_dict)
            for (v in map["values"]) {
		if(v >= 2564) continue
		if(map_dict_reverse[map["values"][v]]){
		    map_dict[map_dict_reverse[map["values"][v]]]["vertex"].push(v)
		}
            }
