...
var wasMoved = false;
function fillItems() {
    jQuery.getJSON('items.json?_=55', function(data) {
	items = data;
...
function addStateDisplay( state ) {
	...
	if (state[i]['type'] == "text") {
        ...
        // make this one invisible - uses new text area for control
        jQuery('#left-down').append("<div id=\"" + gid + "-" + state[i]['name'] +
                "\" parent-id=\"" + parentID + "\" class=\"form-group\" style=\"width: 200px\"><label>" +
                state[i]['name'] +
                "</label><textarea class=\"form-control input-sm state-input\" style=\"display: none;\" type=\"textarea\" rows=\"5\" " +
                "placeholder=\"undefined\" value=\'" +
                text + "\'>" + text + "</textarea></div>");

        //Add textarea link to the ace editor
        var ace_editor = jQuery("#editor-ace").attr("gid", gid + "-" + state[i]['name']);
        editor.session.setValue(text);
        editor.session.on("change",function(e){
            var editor_ace = $("#editor-ace");
            if(editor_ace.attr("gid") == "") return;
            var text = editor.getValue();
            $("#"+editor_ace.attr("gid")+" textarea").attr("value", text);
            $("#"+editor_ace.attr("gid")+" textarea").html(text);
            $( "#"+editor_ace.attr("gid")+" .state-input" ).trigger("change");
            //editor.focus(); 
        });
...
function removeStateDisplay( state ) {
	...
	// remove any existing displays
	jQuery('#'+ gid + '-' + state[i]['name']).remove();
	if(state[i]['type'] == "textarea"){
	    jQuery("#editor-ace").attr("gid","")
...
jQuery(document).ready(function() {
    ...
    right_max = $(window).width()-470;
    editor.setTheme("ace/theme/monokai");
    editor.getSession().setMode("ace/mode/r");
    editor.setFontSize(16);
    editor.setShowPrintMargin(false);
    $("#right").css("resize","horizontal")
    $("#right").css("min-width","20px")
    
    $("#editor-ace").css("left",$("#right").width() + $("#left").width()+20)
    
    $(window).mousemove(function(){
	right_max = $(window).width()-$("#left").width()-100;
	$("#editor-ace").css("left",$("#right").width() + $("#left").width()+20)	
	$("#right").css("max-width",right_max+"px")
	editor.resize()
    });
	...
	if (typeof state['description'] !== "undefined") {
  	    jQuery('#new-description').val( state['description'] );
	} else {
  	    jQuery('#new-description').val( "" );
	}
    });
	...
	if (data['message'] === "permissions denied") {
	    alert("Error: Your user account does not have permissions to request this recipe.");
	    return;
	}
	...
	jQuery('#save-recipe-button').click(function () {
		console.log(" saved in the dialog");
	        ...
	        // add description from interface
     	        state['description'] = jQuery('#new-description').val();
		jQuery.ajax({
